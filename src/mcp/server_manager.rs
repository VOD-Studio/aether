...
use super::config::{ExternalServerConfig, TransportType, McpConfig};
use super::tool_registry::{Tool, ToolDefinition, ToolResult, ToolSource, ToolRegistry};
...
impl McpServer {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let result = match self.config.transport {
            TransportType::Stdio => {
                StdioTransport::new(&self.config).await
                    .map(|t| t.peer().clone())
            }
    ...
    pub async fn call_tool(&self, name: &str, arguments: serde_json::Value) -> Result<ToolResult> {
        ...
        let response = peer.call_tool(params).await
        ...
        if response.is_error.unwrap_or(false) {
            Ok(ToolResult {
                success: false,
                content: String::new(),
                error: Some(response.content.iter().map(|c| format!("{:?}", c)).collect::<Vec<_>>().join("\n")),
            })
        } else {
            Ok(ToolResult {
                success: true,
                content: response.content.iter().map(|c| format!("{:?}", c)).collect::<Vec<_>>().join("\n"),
                error: None,
            })
        }
    }
...
impl Tool for ExternalMcpTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: self.tool.description.as_ref().map(|d| d.to_string()).unwrap_or_default(),
            parameters: serde_json::to_value(self.tool.input_schema.as_ref().clone()).unwrap_or_default(),
        }
    }
    ...
    pub async fn register_all_external_tools(&self) {
        ...
        for (server_name, server) in &self.servers {
            ...
            if server_guard.status() != &ServerStatus::Connected {
                warn!("MCP server {} is not connected, skipping tool registration", server_name);
                continue;
            }
            ...
            match server_guard.list_tools().await {
                Ok(tools) => {
                    ...
                    for tool in tools {
                        let external_tool = ExternalMcpTool::new(
                            server_name.clone(),
                            tool,
                            server.clone(),
                        );
                ...
                Err(e) => {
                    error!("Failed to list tools from MCP server {}: {}", server_name, e);
                }
            }
        }
    }
    ...
    pub async fn reload_config(&mut self, new_config: &McpConfig) -> Result<()> {
        ...
        for server_config in &new_config.external_servers {
            ...
            if let Some(existing) = self.servers.get(&server_config.name) {
                ...
                // 新增服务器
                let server = McpServer::new(server_config.clone());
                self.servers.insert(server_config.name.clone(), Arc::new(RwLock::new(server)));
            }
        }
